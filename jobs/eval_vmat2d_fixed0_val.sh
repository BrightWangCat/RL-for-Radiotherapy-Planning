...
# Need OPENKBP_CASE for env construction
if [[ -f splits/val_cases.txt ]]; then
  export OPENKBP_CASE="$(head -n 1 splits/val_cases.txt)"
else
  echo "Missing splits/val_cases.txt. Run train script once to auto-generate splits."
  exit 1
fi

RUN_DIR="runs/OpenKBPVMAT2D-v0__ppo_discrete_cnn__0__1766557937"

echo "=== fixed-action=0 (val set) ==="
